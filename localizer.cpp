  ...
  float blurring) 
{
  ...
  // your code here (done 3/5)

  //iterates through and updates beliefs with change in x and y
  for (int i = 0; i < height; i++) {
  	  for (int j = 0; j < width; j++) {
	  	  new_i = (i + dy) % height;
		  new_j = (j + dx) % width;
	...
	float p_miss) 
{
	...
	// your code here (done 3/6)
	
	//go through grid and update beliefs based on color
	for (int i = 0; i < height; i++) {
		new_row.clear();
		for (int j = 0; j < width; j++) {
			belief = beliefs[i][j];
			if (color == grid[i][j]) {
				new_row.push_back(belief * p_hit);
			}
			else {
				new_row.push_back(belief * p_miss);
			}
		}
		newGrid.push_back(new_row);
	}
