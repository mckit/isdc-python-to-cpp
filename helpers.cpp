...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// todo (normalize function *done* 3/4) - your code here. need to use the .size() function instead or rows/cols since they arent defined
  	for (int i = 0; i < grid.size(); i++) {
      for (int j = 0; j < grid[0].size(); j++) {
        normalizer += grid[i][j];
      }
    }
  // now need to iterate back through the grid and divide each item by normalizer and append normalized rows to the newGrid
  	for (int i = 0; i < grid.size(); i++) {
     new_row.clear();
      for (int j = 0; j < grid[0].size(); j++) {
      	new_row.push_back(grid[i][j] / normalizer);
      }
...
// your code here (done 3/4)
	...
	float grid_value = 0.0;
  	//window grid to store blurring probabilities
  	vector < vector <float> > window(3, vector <float> (3, 0.0));
  	//assign center_p, corner_p, adjancet_p values to window
  	window[0][0] = corner_p;
  	window[0][1] = adjacent_p;
  	window[0][2] = corner_p;
  	window[1][0] = adjacent_p;
  	window[1][1] = center_p;
  	window[1][2] = adjacent_p;
  	window[2][0] = corner_p;
  	window[2][1] = adjacent_p;
  	window[2][2] = corner_p;

	//calculate new blurred grid, first iterate the grid input with the function
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
		grid_value = grid[i][j];
		//now iterating through the window grid
		for (int dx = -1; dx < window.size(); dx++) {
			for (int dy = -1; dy < window[0].size(); dy++) {
			new_i = (i + dy) % height;
			new_j = (i + dx) % width;
			newGrid[new_i][new_j] += window[dx + 1][dy + 1] * grid_value;
			}
		}
		}
	}
